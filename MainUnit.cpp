...
void __fastcall TMainForm::ButtonAddClick(TObject *Sender) {
	try {
		...
		if (l > a)
			throw Exception(_T("Левое значение должно быть меньше правого модального"));
		if (a > b)
			throw Exception(_T("Левое модальное значение должно быть меньше правого"));
		if (b > r)
			throw Exception(_T("Правое модальное значение должно быть меньше правого значение"));
