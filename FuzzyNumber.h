...
public:
	...
	friend FuzzyNumber operator +(const FuzzyNumber leftArg,
		const FuzzyNumber rightArg)
	{
		return FuzzyNumber(leftArg.m_l + rightArg.m_l,
			leftArg.m_a + rightArg.m_a, leftArg.m_b + rightArg.m_b,
			leftArg.m_r + rightArg.m_r);
	};

	friend FuzzyNumber operator -(const FuzzyNumber leftArg,
		const FuzzyNumber rightArg)
	{
		return FuzzyNumber(leftArg.m_l - rightArg.m_r,
			leftArg.m_a - rightArg.m_b, leftArg.m_b - rightArg.m_a,
			leftArg.m_r - rightArg.m_l);
	};

	friend FuzzyNumber operator* (const double x, const FuzzyNumber rightArg)
	{
		if (x >= 0)
			return FuzzyNumber(x*rightArg.m_l, x*rightArg.m_a, x*rightArg.m_b,
			x*rightArg.m_r);
		else
			return FuzzyNumber(x*rightArg.m_r, x*rightArg.m_b, x*rightArg.m_a,
			x*rightArg.m_l);
	};
